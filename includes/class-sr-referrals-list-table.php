...
class SR_Referrals_List_Table extends WP_List_Table {
    ...
    protected function column_username( $item ) {
        ...
        $actions = array(
            'edit' => sprintf( '<a href="%s">%s</a>', $edit_url, __( 'Edit', 'smart-referrals' ) ),
        );
    ...
    protected function column_actions( $item ) {
        $generate_url = add_query_arg(
            array(
                ...
                'user_id'  => $item->ID,
                '_wpnonce' => wp_create_nonce( 'generate_referral_' . $item->ID ),
            ),
            admin_url( 'admin.php?page=sr-referrals' )
        );

        return sprintf(
            '<a href="%s" class="button">%s</a>',
            esc_url( $generate_url ),
            __( 'Generate Referral', 'smart-referrals' )
        );
    }
